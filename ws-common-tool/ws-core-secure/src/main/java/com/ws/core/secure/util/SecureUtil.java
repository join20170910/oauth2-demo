package com.ws.core.secure.util;

import com.ws.core.launch.constant.TokenConstant;
import com.ws.core.secure.TokenInfo;
import com.ws.core.secure.WsUser;
import com.ws.core.secure.constant.RoleConstant;
import com.ws.core.secure.constant.SecureConstant;
import com.ws.core.secure.exception.SecureException;
import com.ws.core.secure.provider.IClientDetails;
import com.ws.core.secure.provider.IClientDetailsService;
...
public class SecureUtil {
	 ...
	 * @return wsUser
	 */
	public static WsUser getUser() {
		HttpServletRequest request = WebUtil.getRequest();
		if (request == null) {
			return null;
		}
		// 优先从 request 中获取
		Object wsUser = request.getAttribute(BLADE_USER_REQUEST_ATTR);
		if (wsUser == null) {
			wsUser = getUser(request);
			if (wsUser != null) {
				// 设置到 request 中
				request.setAttribute(BLADE_USER_REQUEST_ATTR, wsUser);
			}
		}
		return (WsUser) wsUser;
	}
	 ...
	 * @return wsUser
	 */
	public static WsUser getUser(HttpServletRequest request) {
		...
		wsUser.setClientId(clientId);
		wsUser.setUserId(userId);
		wsUser.setTenantId(tenantId);
		wsUser.setAccount(account);
		wsUser.setRoleId(roleId);
		wsUser.setRoleName(roleName);
